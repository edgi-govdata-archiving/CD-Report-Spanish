...
filenames = list.files(here("../CD_Dirs/VT"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*-[^-]*)-.*$","\\1", i)
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/VT", i)))
  # cat( "file is ", name, "\n" )
}

activefacilities <- `active-facilities_All_pg3_VT`
...
VT <- housemembers[which(housemembers$CD=="VT"), ]
...
##### El distrito de Vermont; asiento ocupado por `r VT[1,2]` desde `r VT[1,8]` 
 <div class="container">
  <img src="../CD_maps/VT_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_VT`
...
enforcement <- `enforcements_All_pg3_VT`
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="VT", "VT", "otros CDs/\nestados")
...
VT_dot <- CWAper1000[which(CWAper1000$CD.State=="VT"), ]
...
ggsave("dotplot_VT.png", dotplot, path=here("../CD_Dirs/VT"), scale = 2, width=9, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/VT/dotplot_VT.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En VT, hubo <span style="color:#569b5e">`r VT_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for VT 0
CAArecurring <- `recurring-violations_All_pg3_VT`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("VT")
CAArecurring$Region2 <- as.character("All of VT")
CAArecurring[2,4] <- as.character("VT")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for VT 0
CWArecurring <- `recurring-violations_All_pg3_VT`[-c(1,3,4,6),]
CWArecurring <- CWArecurring %>%
  select(State,Facilities, Percent)
names(CWArecurring)[1] <- "Region"
CWArecurring[1,1] <- as.character("Rest of State")
CWArecurring[2,1] <- as.character("VT")
CWArecurring$Region2 <- as.character("All of VT")
CWArecurring[2,4] <- as.character("VT")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for VT 0
RCRArecurring <- `recurring-violations_All_pg3_VT`[-c(1,2,4,5),]
RCRArecurring <- RCRArecurring %>%
  select(State,Facilities, Percent)
names(RCRArecurring)[1] <- "Region"
RCRArecurring[1,1] <- as.character("Rest of State")
RCRArecurring[2,1] <- as.character("VT")
RCRArecurring$Region2 <- as.character("All of VT")
RCRArecurring[2,4] <- as.character("VT")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_VT`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_VT`
...
```
# ASPECTOS DESTACADOS DEL DISTRITO DE VERMONT
...
names(`inspectionsper1000_All_pg4_VT`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_VT`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_VT`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_VT`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_VT`)
...
names(`violationsper1000_All_pg4_VT`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_VT`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_VT`)
...
CAAbadactors <- `noncomp_CAA_pg6_VT`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/VT"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_VT`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/VT"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_VT`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/VT"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/VT/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/VT/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/VT/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_VT` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_VT`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_VT`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_VT`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_VT`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_VT`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_VT`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_VT`
...
<br />
<a ><img src="../CD_images/VT_rep.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r VT[1,2]`](`r VT[1,9]`) (`r VT[1,3]`)</h2>

## Ha estado en esta oficina `r VT[1,6]` `r VT[1,7]`, `r VT[1,8]`
...
<h4><span style="padding-left:30px">[`r VT[1,10]`](`r VT[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r VT[1,12]`](`r VT[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r VT[1,14]`](`r VT[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r VT[1,16]`](`r VT[1,17]`)</span></h4>
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/VT_2020.pdf)
