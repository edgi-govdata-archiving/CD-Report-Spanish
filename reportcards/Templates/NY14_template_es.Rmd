...
CWAper1000 <- violationsper1000_CWA500
...
filenames = list.files(here("../CD_Dirs/NY14"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/NY14", i)))
}

activefacilities <- `active-facilities_All_pg3_NY-14`
...
theme_meg <- function () { 
  theme_bw(base_family="Georgia") %+replace% 
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.border = element_blank(),
          ...
          axis.title.y=element_text(face = "bold", size = 7, angle=90), plot.title=element_text(face = "bold", size = 12, hjust =.5), 
...
##### El 14$^\circ$ distrito de New York; asiento ocupado por `r leg_info$name` desde `r leg_info$since_year` 
 <div class="container">
  <img src="../CD_maps/NY14_map.png" alt="Snow" style="width:100%;">
   <div class="bottom-left">El mapa demuestra el Distrito Congresual o Estado con grupos de empresas reguladas</div>
  </div>
...
El Congreso encarga a la Agencia de Protección Ambiental (EPA) que aplique las leyes que protegen a las personas de la contaminación del aire, la contaminación del agua y los desechos peligrosos. Sin aplicación y cumplimiento efectivo, estas leyes son inservibles. Esta boleta de calificaciones está basada en datos de la base de datos del Historial de Cumplimiento y Aplicación en Línea (ECHO) de la EPA. La boleta de revisa las violaciones, inspecciones y acciones de aplicación bajo tres leyes: Ley de Aire Limpio (CAA), Ley de Agua Limpia (CWA) y Ley de Conservación y Recuperación de Recursos (RCRA) para este Distrito Congresual desde 2001. Estas boletas están disponibles en el sitio web de EEW para todos los miembros de los comités del Congreso que supervisan la EPA: el Comité de Energía y Comercio de la Cámara de Representantes y el Comité de Obras Públicas y Medio Ambiente del Senado. Actualmente, EEW está trabajando para desarrollar boletas de calificaciones para todos los estados y distritos congresuales de los EE. UU. El sitio web de EEW también tiene un análisis de las tendencias de aplicación de leyes ambientales a nivel nacional e información sobre los problemas de datos. Estas boletas contienen datos de las agencias ambientales estatales y también de la EPA. Siempre que los estados
hagan cumplir las leyes ambientales, es porque la EPA les ha delegado esa autoridad. Esto significa que la EPA debe asegurarse de que los estados estén haciendo su trabajo y el Congreso debe asegurarse de que la EPA esté haciendo su trabajo. El público debe tener datos precisos de los estados y de la EPA para entender si las leyes ambientales nacionales se están aplicando correctamente. Por primera vez, las boletas de calificaciones a nivel Congresual de EEW brindan a los congresistas y a sus electores la oportunidad de evaluar si la EPA está cumpliendo con su mandato en su distrito. El Congreso puede fortalecer la aplicación de la EPA aumentando su presupuesto, aprobando leyes más efectivas, requiriendo una mejor recopilación de datos y haciendo responsable a la EPA cuando no protege a las personas.

## ¿Qué es una “empresa regulada”?

<a ><img src="../logosandimages/facilities2_es.png" style="float: center; width: 70%; margin-left: 15%" ></a>

<br />

En este informe, una empresa regulada es una empresa que reporta las emisiones al aire o al agua según la Ley de Aire Limpio o la Ley de Agua Limpia, o una empresa que genera, transporta y elimina desechos peligrosos según la Ley de Conservación y Recuperación de Recursos. Las empresas reguladas pueden ser a gran escala, como las refinerías de petróleo, o a pequeña escala como las tintorerías.
...
# LOS CAMBIOS PRINCIPALES EN ESTE DISTRITO BAJO LA ADMINISTRACIÓN DE TRUMP

```{r echo=FALSE}

##getting %difference CWA violations
CWAviolations <- `violations_CWA_pg6_NY-14`
...
vCWAchange$Change <- ifelse(vCWAchange$PercentDifference<0, "peor", "mejor")

vCWAchange$Positive <- abs(vCWAchange$PercentDifference)
vCWAchange$Value <- ifelse(vCWAchange$PercentDifference>=0, "un aumento", "una disminución")

#enforcement % difference
enforcement <- `enforcements_All_pg3_NY-14`
...
actionchange$Extra <- ifelse((actionchange$PercentDifference>=100)|(actionchange$PercentDifference<=-100), "mucho", "")
actionchange$Change <- ifelse(actionchange$PercentDifference<0, "peor", "mejor")

actionchange$Positive <- abs(actionchange$PercentDifference)
actionchange$Value <- ifelse(actionchange$PercentDifference>=0, "un aumento", "una disminución")

```

<h3>Las violaciones de la Ley de Agua Limpia *: son <span style="color:#569b5e">`r vCWAchange[1,4]` `r vCWAchange[1,5]`</span> que el promedio de los 16 años anteriores, lo que representa <span style="color:#569b5e">`r vCWAchange[1,7]` de `r vCWAchange[1,6]`% </span> en violaciones</h3>

<br />

<h3>Las acciones de aplicación bajo la Ley de Aire Limpio, la Ley de Agua Limpia y la Ley de Conservación y Recuperación de Recursos*: son <span style="color:#569b5e">`r actionchange[1,4]` `r actionchange[1,5]`</span> que el promedio durante los 16 años anteriores, lo que representa <span style="color:#569b5e">`r actionchange[1,7]` de `r actionchange[1,6]`% </span> en las acciones de aplicación</h3>
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="NY14", "NY14", "otros CDs/\nestados")
...
NY14_dot <- CWAper1000[which(CWAper1000$CD.State=="NY14"), ]
...
dotplot <-ggplot(CWAper100, aes(x=Region, y=Per100, fill=Highlight))+
  ...
  theme(axis.line.x=element_blank(), axis.title.x=element_blank(),
        legend.title = element_blank(), axis.text.x=element_text(size=10, hjust=.6), legend.position = c(0.15,.8), axis.ticks.x=element_blank())+scale_x_discrete(labels=c("Distritos Congresuales","Estados"))
ggsave("dotplot_NY14.png", dotplot, path=here("../CD_Dirs/NY14"), scale = 2, width=9, height=5, units=c("cm"))
...
if ( nchar( trim( cd_outliers ) < 2 )) {
  cd_outliers <- paste( 'Distritos Congresuales: ', cd_outliers )
}
if ( nchar( trim( state_outliers ) < 2 )) {
  state_outliers <- paste( 'Estados: ', state_outliers )
}

```
 
<a ><img src="../CD_Dirs/NY14/dotplot_NY14.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>

*Este es un promedio, no significa que todas las empresas tengan una violación. Algunas empresas pueden tener muchas más violaciones o cero violaciones.
<br />
<br />

<h3>En NY14, hubo <span style="color:#569b5e">`r NY14_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 empresas reguladas en 2019.</h3>
<br />

<h2> Valores atípicos (no mostrados):  Los distritos y estados con mas de 800 violaciones por cada 100 empresas</h2>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for NY 14
CAArecurring <- `recurring-violations_All_pg3_NY-14`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("NY 14")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("NY 14")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_NY-14`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_NY-14`
...
inspectiongraph <- ggplot(inspections, mapping=aes(x=factor(Date),
  ...
  scale_fill_manual(values=c("#450d54", "#22908d", "#e56d13"))+
  labs(y="Inspecciones", x="Año", title="Inspecciones de empresas-CAA, CWA, RCRA\n", subtitle = "Datos potencialmente incompletos")+
...
#syntax for bullet points
enforcementchange$CountChange <- ifelse(enforcementchange$PercentDifferenceCount>=0, "aumentar", "disminución")
enforcementchange$PositiveCount <- abs(enforcementchange$PercentDifferenceCount)
enforcementchange$FineChange <- ifelse(enforcementchange$PercentDifferenceFine>=0, "aumentar", "disminución")
...
```
# ASPECTOS DESTACADOS DEL 14$^\circ$ DISTRITO DE NEW YORK
...
* <h3>Bajo la Ley de Agua Limpia, la ley cuya regulación está mejor documentada por los datos disponibles de la EPA, <span style="color:#569b5e">`r CWArecurring[2,2]` empresas, que representan el `r CWArecurring[2,3]`% de todas las empresas reguladas en NY14,</span> estuvieron en violación durante <span style="color:#569b5e">al menos 9 meses de los últimos 3 años.</span></h3>
<br/>
...
USAinspectionsper1000_All$Region <- as.factor("Nivel Nacional")
names(`inspectionsper1000_All_pg4_NY-102320.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NY-102320.csv`$Region <- as.factor("Este Estado")
names(`inspectionsper1000_All_pg4_NY-14`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NY-14`$Region <- as.factor("Este Distrito Congresual")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_NY-102320.csv`, `inspectionsper1000_All_pg4_NY-14`)
...
inspect1000bar <- ggplot(inspectionsper1000, aes(fill=Region, y=Inspectionsper1000, x=Program, alpha=Program))+
  ...
  ggtitle("Inspecciones por cada 1000 empresas en 2019")+
  scale_y_continuous(expand=c(0,0),limits=c(0,1350))+geom_hline(yintercept=1000, linetype="dashed", color="black")+geom_text(aes(label="Promedio de 1 inspección por empresa",family="Georgia"),x=2,y=1050,color="black",size=4)+
  theme_meg()
...
names(`violationsper1000_All_pg4_NY-102320.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NY-102320.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_NY-14`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NY-14`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_NY-102320.csv`, `violationsper1000_All_pg4_NY-14`)
...
violations1000bar <- ggplot(violationsper1000, aes(fill=Region, y=violationsper1000, x=Program, alpha=Program))+
  ...
  ggtitle("Violaciones por cada 1000 empresas en 2019")+
...
Estos dos gráficos muestran cómo las inspecciones y violaciones en este distrito se comparan con los promedios nacionales y estatales por cada 1000 empresas en 2019. Usamos datos de 2019 ya que fue el año completo más reciente y la base de datos de ECHO solo informa las empresas actualmente activas. Para poder comparar entre ubicaciones con un número diferente de empresas activas, estandarizamos la comparación a un valor por cada 1000 empresas, ajustando proporcionalmente los datos si hay más o menos de 1000 empresas en un distrito o estado. 

Para acceder el Jupyter notebook que extrae datos de ECHO a nivel estatal y al nivel de Distrito Congresual, haga clic [aquí](https://colab.research.google.com/github/edgi-govdata-archiving/ECHO-Cross-Program/blob/master/Spanish_AllPrograms.ipynb). Para obtener datos nacionales, haga clic [aquí](https://colab.research.google.com/github/edgi-govdata-archiving/ECHO-Cross-Program/blob/master/Spanish_ECHO_National.ipynb).
...
<h1><span style="margin-bottom:0px; padding-bottom:0px">Incumplimiento Reciente en este Distrito</span></h1>
<font size="2">Estos gráficos demuestran las diez empresas en este distrito con el peor historial de cumplimiento de leyes ambientales basado en el número de trimestres que no han cumplido con las leyes ambientales en los últimos 3 años (estos trimestres no son necesariamente consecutivos).</font>
```{r, fig.height = 7, fig.width = 5, fig.align = "left"}
#call in data and clean up 
CAAbadactors <- `noncomp_CAA_pg6_NY-14`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/NY14"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_NY-14`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/NY14"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_NY-14`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/NY14"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
```
`r if ( nrows_caa == 0) { 
    "<br /><h3>No se informa de violadores consistentes de la Ley de Aire Limpio (CAA) en ECHO</h3>"
  } else {
    "<br /><a ><img src=\"../CD_Dirs/NY14/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    "<br /><h3>No se informa de violadores consistentes de la Ley de Conservación y Recuperación de Recursos (RCRA) en ECHO</h3>"
  } else {
    "<br /><a ><img src=\"../CD_Dirs/NY14/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    "<br /><h3>No se informa de violadores consistentes de la Ley de Agua Limpia (CWA) en ECHO</h3>"
  } else {
    "<br /><a ><img src=\"../CD_Dirs/NY14/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
<h2>Actualmente hay `r activefacilities[1,2] `  empresas que se reportan a la `r activefacilities[1,1]` en este distrito.</h2>
```{r, fig.height = 5, fig.width = 8, fig.align = "center"}

#clean up data
CAAviolations <- `violations_CAA_pg5_NY-14` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_NY-14`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_NY-14`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_NY-14`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_NY-14`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_NY-14`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_NY-14`
...
<br />
<a ><img src="../CD_images/NY14_rep.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>

<h2>[`r leg_info$name`](`r leg_info$wikipedia_url`) (Demócrata)</h2>

## Ha estado en esta oficina desde el enero 3 del 2019 

### [Página de Govtrack](`r leg_info$govtrack_url`)
...
### Este miembro del Congreso sirve en los siguientes comités:  
...
* ECHO no registra cuántas empresas reguladas correspondian a cada programa en los años anteriores. Por lo tanto, no podemos calcular la cantidad de inspecciones, acciones de aplicación y violaciones por empresa regulada antes de 2019.
...
EEW ha elaborado boletas de calificaciones para los 76 senadores y representantes de la Cámara que forman parte del Comité de Energía y Comercio de la Cámara y del Comité de Obras Públicas y Medio Ambiente del Senado, con el objetivo de hacerlos llegar a los representantes y senadores que supervisan la EPA, ya que estos comités son responsables de la supervisión de la EPA. En un futuro cercano, EEW planea desarrollar boletas de calificaciones congresuales para todos los estados y distritos congresuales del país. Al aportar una perspectiva nueva sobre el estado crónico de incumplimiento en sus estados y distritos, esperamos proporcionar a estos representantes con la información que necesitan para evaluar el estado de cumplimento y la aplicación de leyes ambientales en sus comunidades para que puedan responsabilizar a la EPA de una manera más efectiva.
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/NY14_2020_es.pdf)
