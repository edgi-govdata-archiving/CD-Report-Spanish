...
filenames = list.files(here("../CD_Dirs/TX"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/TX", i)))
}

activefacilities <- `active-facilities_All_pg3_TX`
...
##### El estado de Texas; asiento ocupado por `r sen1_name` desde `r sen1_since` y `r sen2_name` desde `r sen2_since`
 <div class="container">
  <img src="../State_maps/TX_map.png" alt="Snow" style="width:100%;">
...
<h4>INFORME GENERADO EL 3.3.2021<span style="padding-left:350px">PÁGINA 1</span></h4>
...
CWAviolations <- violations_CWA_pg6_TX
...
vCWAchange$Value <- ifelse(vCWAchange$PercentDifference>=0, "aumento del", "disminucion del")

#enforcement % difference
enforcement <- enforcements_All_pg3_TX
...
actionchange$Value <- ifelse(actionchange$PercentDifference>=0, "aumento del", "disminucion del")
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="TX", "TX", "otros CDs/\nestados")
...
TX_dot <- CWAper1000[which(CWAper1000$CD.State=="TX"), ]
...
ggsave("dotplot_TX.png", dotplot, path=here("../CD_Dirs/TX"), scale = 2, width=9, height=5, units=c("cm"))
...
<a ><img src="../CD_Dirs/TX/dotplot_TX.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En TX, hubo <span style="color:#569b5e">`r TX_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
<br />
...
<hr style="height:3px;border-width:0;color: #C1E0D7;background-color: #C1E0D7; margin-top: 2em">
<h4>Report Generated on 10.2.2020<span style="padding-left:400px">PÁGINA 3</span></h4>
```{r echo=FALSE}
#CAA

##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for TX
CAArecurring <- `recurring-violations_All_pg3_TX`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("TX")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("TX")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for TX
CWArecurring <- `recurring-violations_All_pg3_TX`[-c(1,3),]
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_TX`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_TX`
...
#syntax for bullet points
enforcementchange$CountChange <- ifelse(enforcementchange$PercentDifferenceCount>=0, "aumento del", "disminucion del")
enforcementchange$PositiveCount <- abs(enforcementchange$PercentDifferenceCount)
enforcementchange$FineChange <- ifelse(enforcementchange$PercentDifferenceFine>=0, "aumento del", "disminucion del")
...
enforcementactiongraph <- ggplot(enforcement, mapping=aes(x=factor(Date),y=Count, fill=President, group=1))+
  ...
  scale_y_continuous(expand=c(0,0), limits=c(0,max(effluentviolations$Violations+(.2*max(effluentviolations$Violations)))))+
  scale_x_discrete(breaks=c(2004,2008,2012,2016,2019))+
  geom_text(aes(label=Count, family="Georgia", fontface="bold"), position = position_dodge(0.9), vjust = -0.1, size=1.8)+
  theme_meg2()+
  theme(axis.text.x=element_text(size=9),
        axis.title.x = element_blank(), plot.subtitle = element_text(hjust=.5, size=9), plot.title = 
          element_text(lineheight = .1, size=10))
...
```
# ASPECTOS DESTACADOS DEL ESTADO DE TEXAS
...
names(`inspectionsper1000_All_pg4_TX`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_TX`$Region <- as.factor("State")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_TX`)
...
names(`violationsper1000_All_pg4_TX`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_TX`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_TX`)
...
CAAbadactors <- `noncomp_CAA_pg6_TX`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/TX"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_TX`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/TX"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_TX`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/TX"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_TX` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_TX`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_TX`
...
<h2>Actualmente hay `r activefacilities[2,2] ` instalaciones que se informan bajo 
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_TX`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_TX`
...
<h2>Actualmente hay `r activefacilities[3,2] `  instalaciones que se reportan bajo `r activefacilities[3,1]` en este estado.</h2>
```{r, fig.height = 5, fig.width = 8, fig.align = "center"}

#clean up data
RCRAviolations <- `violations_RCRA_pg7_TX`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_TX`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_TX`
...
<br />
<a ><img src="../CD_images/TX_sen`r senators[1,"sen_class"]`.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r senators$name[1]`](`r senators$wikipedia_url[1]`) (`r senators$party[1]`)</h2>

## Ha estado en esta oficina desde  `r senators$since_date[1]`

### [Página de GovTrack](`r senators$govtrack_url[1]`)
...
# INFORMACIÓN DEL LEGISLADOR
<br />
<a ><img src="../CD_images/TX_sen`r senators[2,"sen_class"]`.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>
...
<hr style="height:3px;border-width:0;color: #C1E0D7;background-color: #C1E0D7; margin-top: 2em">
<h4>INFORME GENERADO EL 12.13.2020<span style="padding-left:350px">PÁGINA 14</span></h4>
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/TX_2020.pdf)
