...
filenames = list.files(here("../CD_Dirs/NM1"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/NM1", i)))
}

activefacilities <- `active-facilities_All_pg3_NM-1`
...
##### El 1$^\circ$ distrito de New Mexico; asiento ocupado por `r leg_info$name` desde `r leg_info$since_year` 
 <div class="container">
  <img src="../CD_maps/NM1_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_NM-1`
...
enforcement <- `enforcements_All_pg3_NM-1`
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="NM1", "NM1", "otros CDs/\nestados")
...
NM1_dot <- CWAper1000[which(CWAper1000$CD.State=="NM1"), ]
...
ggsave("dotplot_NM1.png", dotplot, path=here("../CD_Dirs/NM1"), scale = 2, width=9, height=5, units=c("cm"))
...
<a ><img src="../CD_Dirs/NM1/dotplot_NM1.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En NM1, hubo <span style="color:#569b5e">`r NM1_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
<br />
...
#ok so We'll put the % in the title for the state, and try to figure out the same for NM 1
CAArecurring <- `recurring-violations_All_pg3_NM-1`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("NM 1")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("NM 1")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_NM-1`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_NM-1`
...
```
# ASPECTOS DESTACADOS DEL 1$^\circ$ DISTRITO DE NEW MEXICO
...
names(`inspectionsper1000_All_pg4_NM-111820.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NM-111820.csv`$Region <- as.factor("Este Estado")
names(`inspectionsper1000_All_pg4_NM-1`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NM-1`$Region <- as.factor("Este Distrito Congresual")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_NM-111820.csv`, `inspectionsper1000_All_pg4_NM-1`)
...
names(`violationsper1000_All_pg4_NM-111820.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NM-111820.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_NM-1`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NM-1`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_NM-111820.csv`, `violationsper1000_All_pg4_NM-1`)
...
CAAbadactors <- `noncomp_CAA_pg6_NM-1`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/NM1"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_NM-1`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/NM1"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_NM-1`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/NM1"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/NM1/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/NM1/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/NM1/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_NM-1` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_NM-1`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_NM-1`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_NM-1`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_NM-1`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_NM-1`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_NM-1`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_NM-1`
...
<br />
<a ><img src="../CD_images/NM1_rep.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>
...
## Nombre del comité: Comité de Servicios Armados de la Cámara de Representantes 
Jurisdicción: El Comité de Servicios Armados de la Cámara de Representantes tiene jurisdicción legislativa sobre asuntos militares y asuntos de defensa.
<br />
[Página web del comité](https://armedservices.house.gov/)
<br /> <br /> 

### Subcomité: Personal militar, Rango: 6 <br /> 

### Subcomité: Preparación, rango: 10

## Nombre del comité: Comité de Recursos Naturales de la Cámara de Representantes
Jurisdicción: El Comité de Recursos Naturales de la Cámara de Representantes considera la legislación sobre la producción de energía estadounidense, las tierras minerales y la minería, la pesca y la vida silvestre, las tierras públicas, los océanos, los nativos americanos, el riego y la recuperación de tierras.
<br />
[Página web del comité](https://naturalresources.house.gov/)
<br /> <br /> 

### Subcomité: Parques Nacionales, Bosques y Tierras Públicas, Rango: 1 <br /> 

### Subcomité: Subcomité para los Pueblos Indígenas de los Estados Unidos, Rango: 4
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/NM1_2020.pdf)
