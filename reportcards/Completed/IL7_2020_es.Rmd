...
filenames = list.files(here("../CD_Dirs/IL7"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/IL7", i)))
}

activefacilities <- `active-facilities_All_pg3_IL-7`
...
##### El 7$^\circ$ distrito de Illinois; asiento ocupado por `r leg_info$name` desde `r leg_info$since_year` 
 <div class="container">
  <img src="../CD_maps/IL7_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_IL-7`
...
enforcement <- `enforcements_All_pg3_IL-7`
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="IL7", "IL7", "otros CDs/\nestados")
...
IL7_dot <- CWAper1000[which(CWAper1000$CD.State=="IL7"), ]
...
ggsave("dotplot_IL7.png", dotplot, path=here("../CD_Dirs/IL7"), scale = 2, width=9, height=5, units=c("cm"))
...
<a ><img src="../CD_Dirs/IL7/dotplot_IL7.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En IL7, hubo <span style="color:#569b5e">`r IL7_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
<br />
...
#ok so We'll put the % in the title for the state, and try to figure out the same for IL 7
CAArecurring <- `recurring-violations_All_pg3_IL-7`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("IL 7")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("IL 7")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_IL-7`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_IL-7`
...
```
# ASPECTOS DESTACADOS DEL 7$^\circ$ DISTRITO DE ILLINOIS
...
names(`inspectionsper1000_All_pg4_IL-7`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_IL-7`$Region <- as.factor("Este Distrito Congresual")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_IL-111820.csv`, `inspectionsper1000_All_pg4_IL-7`)
...
names(`violationsper1000_All_pg4_IL-7`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_IL-7`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_IL-111820.csv`, `violationsper1000_All_pg4_IL-7`)
...
CAAbadactors <- `noncomp_CAA_pg6_IL-7`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/IL7"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_IL-7`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/IL7"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_IL-7`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/IL7"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/IL7/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/IL7/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/IL7/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_IL-7` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_IL-7`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_IL-7`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_IL-7`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_IL-7`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_IL-7`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_IL-7`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_IL-7`
...
<br />
<a ><img src="../CD_images/IL7_rep.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>
...
## Nombre del comité: Comité de Medios y Arbitrios de la Cámara de Representantes 
Jurisdicción: El Comité de Medios y Arbitrios es el comité principal de redacción de impuestos en la Cámara de Representantes. El Comité deriva una gran parte de su jurisdicción del Artículo I, Sección VII de la Constitución de los Estados Unidos, que declara: "Todos los proyectos de ley para recaudar ingresos se originarán en la Cámara de Representantes". El comité tiene jurisdicción sobre los ingresos y cuestiones relacionadas, como los aranceles, los acuerdos comerciales recíprocos y la deuda consolidada de los Estados Unidos. También supervisa los aspectos relacionados con los ingresos del sistema de Seguridad Social, Medicare y los programas de servicios sociales.
<br />
[Página web del comité](https://waysandmeans.house.gov/)
<br /> <br /> 

### Subcomité: Apoyo a los trabajadores y las familias, Rango: 1 <br /> 

### Subcomité: Comercio, Rango: 4
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/IL7_2020.pdf)
