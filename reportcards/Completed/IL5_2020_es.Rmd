...
filenames = list.files(here("../CD_Dirs/IL5"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/IL5", i)))
}

activefacilities <- `active-facilities_All_pg3_IL-5`
...
##### El 5$^\circ$ distrito de Illinois; asiento ocupado por `r leg_info$name` desde `r leg_info$since_year` 
 <div class="container">
  <img src="../CD_maps/IL5_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_IL-5`
...
enforcement <- `enforcements_All_pg3_IL-5`
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="IL5", "IL5", "otros CDs/\nestados")
...
IL5_dot <- CWAper1000[which(CWAper1000$CD.State=="IL5"), ]
...
ggsave("dotplot_IL5.png", dotplot, path=here("../CD_Dirs/IL5"), scale = 2, width=9, height=5, units=c("cm"))
...
<a ><img src="../CD_Dirs/IL5/dotplot_IL5.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En IL5, hubo <span style="color:#569b5e">`r IL5_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
<br />

<h2> Valores atípicos (no mostrados):  Los distritos y estados con mas de 800 violaciones por cada 100 instalaciones</h2>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for IL 5
CAArecurring <- `recurring-violations_All_pg3_IL-5`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("IL 5")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("IL 5")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_IL-5`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_IL-5`
...
```
# ASPECTOS DESTACADOS DEL 5$^\circ$ DISTRITO DE ILLINOIS
...
* <h3>Bajo la Ley de Agua Limpia, la ley cuya regulación está mejor documentada por los datos disponibles de la EPA, <span style="color:#569b5e">`r CWArecurring[2,2]` instalaciones, que representan el `r CWArecurring[2,3]`% de todas las instalaciones reguladas en IL5,</span> estuvieron en violación durante <span style="color:#569b5e">al menos 9 meses de los últimos 3 años.</span></h3>
<br/>
...
names(`inspectionsper1000_All_pg4_IL-111820.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_IL-111820.csv`$Region <- as.factor("Este Estado")
names(`inspectionsper1000_All_pg4_IL-5`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_IL-5`$Region <- as.factor("Este Distrito Congresual")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_IL-111820.csv`, `inspectionsper1000_All_pg4_IL-5`)
...
names(`violationsper1000_All_pg4_IL-111820.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_IL-111820.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_IL-5`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_IL-5`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_IL-111820.csv`, `violationsper1000_All_pg4_IL-5`)
...
<font size="2">Estos gráficos demuestran las diez instalaciones en este distrito con el peor historial de cumplimiento de leyes ambientales basado en el número de trimestres que no han cumplido con las leyes ambientales en los últimos 3 años (estos trimestres no son necesariamente consecutivos).</font>
```{r, fig.height = 7, fig.width = 5, fig.align = "left"}
#call in data and clean up 
CAAbadactors <- `noncomp_CAA_pg6_IL-5`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/IL5"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_IL-5`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/IL5"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_IL-5`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/IL5"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/IL5/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/IL5/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/IL5/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
<h2>Actualmente hay `r activefacilities[1,2] `  instalaciones que se reportan a la `r activefacilities[1,1]` en este distrito.</h2>
```{r, fig.height = 5, fig.width = 8, fig.align = "center"}

#clean up data
CAAviolations <- `violations_CAA_pg5_IL-5` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_IL-5`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_IL-5`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_IL-5`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_IL-5`
...
<h2>Hay `r activefacilities[3,2] `  instalaciones que actualmente se reportan bajo `r activefacilities[3,1]` en este distrito.</h2>
```{r, fig.height = 5, fig.width = 8, fig.align = "center"}

#clean up data
RCRAviolations <- `violations_RCRA_pg7_IL-5`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_IL-5`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_IL-5`
...
<br />
<a ><img src="../CD_images/IL5_rep.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>
...
## Nombre del comité: Comité Permanente de Inteligencia de la Cámara de Representantes 
Jurisdicción: El Comité Permanente de Inteligencia de la Cámara de Representantes de los Estados Unidos (HPSCI) es un comité de la Cámara de Representantes de los Estados Unidos, actualmente presidido por el congresista Devin Nunes (California). Creada en 1977, la HPSCI está a cargo de la supervisión de la Comunidad de Inteligencia de los Estados Unidos, que incluye las actividades de inteligencia y las actividades relacionadas con la inteligencia de los
siguientes diecisiete elementos del gobierno de los Estados Unidos, y el Programa de Inteligencia Militar.
<br />
[Página web del comité](https://intelligence.house.gov/)
<br /> <br /> 

### Subcomité: Tecnologías estratégicas e investigación avanzada (STAR), Rango: 3 <br /> 

### Subcomité: Contraterrorismo, Contrainteligencia y Contraproliferación (C3), Rango: 3

## Nombre del comité: Comité de Asignaciones de la Cámara de Representantes 
Jurisdicción: El Comité de Asignaciones de la Cámara de Representantes es responsable de la legislación que asigna los fondos federales antes de los gastos del tesoro. Las asignaciones generalmente se limitan a los niveles establecidos por la Resolución de Presupuesto redactada por el Comité de Presupuesto de la Cámara. El Comité también es responsable de las facturas de gastos suplementarios, que a veces se necesitan a la mitad de un año fiscal para compensar los gastos de emergencia.
<br />
[Página web del comité](https://appropriations.house.gov/)
<br /> <br /> 

### Subcomité: Interior, Medio Ambiente y Agencias Relacionadas, Rango: 5 <br /> 

### Subcomité: Transporte, Vivienda y Desarrollo Urbano y Agencias Relacionadas, Rango: 2 <br /> 

### Subcomité: Servicios Financieros y Gobierno General, Rango: 1
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/IL5_2020.pdf)
